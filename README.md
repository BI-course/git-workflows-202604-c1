[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/6B-rL6oS) 

# Collaborative Git Workflows

| Key              | Value                                                                                                                                                                                                                                                        |
|:-----------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Course Codes** | BBT 4106                                                                                                                                                                                                                     |
| **Course Names** | BBT 4106: Business Intelligence I (Week 1-3 of 13)                                                                                                                                                                             |
| **Semester**     | April to July 2026                                                                                                                                                                                                           |
| **Lecturer**     | Allan Omondi                                                                                                                                                                                                                 |
| **Contact**      | aomondi@strathmore.edu                                                                                                                                                                                                       |
| **Note**         | The lecture contains both theory and practice.<br/>This notebook forms part of the practice.<br/>It is intended for educational purpose only.<br/>Recommended citation: [BibTex](https://raw.githubusercontent.com/course-files/Git/refs/heads/main/RecommendedCitation.bib) |

## Group Members

| Registration Number | Name |
|:--------------------|:------------------|
| 160819 | Alfred Njaku |
| 163604 | Urvi Patel |
| 166567 | Nicole Wachira |
| 168051 | Joy Mbote |
| 169302 | Anastacia Muya |
| 170214 | Chelang'at Grace |

## Task Allocation (Issues & Milestones)

| Member | Issue | Title | Description | Progress |
|:--------|:------|:------|:------------|:----------|
|Grace Chelangat | Issue #1 | feature/lab-1/update-project-README | Update the project's README with team roles | 50% Complete |
| Joy Mbote | Issue #2 | feature/lab-1/research-on-data-sources | Create a data_source.md file reviewing sources of data in a business | 50% Complete |
| Urvi Patel | Issue #3 | feature/lab-1/research-on-star-schema | Create a warehouse_schema.md describing a Star Schema | 50% Complete |
| Alfred Njaku | Issue #4 | feature/lab-1/research-on-ETL-ELT-EtLT | Create a data_pipeline.md differentiating between ETL, ELT, and EtLT in the context of compliance with legal requirements in an industry | 75% Complete |
| Nicole Wachira | Issue #4 | feature/lab-1/differentiating-on-ETL-ELT-EtLT | Create a data_pipeline.md differentiating between ETL, ELT, and EtLT in the context of compliance with legal requirements in an industry | 75% Complete |
| Anastacia Muya | Issue #5 | feature/lab-1/research-on-data-governance | Add a governance.md file reviewing data governance and access to PII | 100% Complete |




## Lab Manuals

Refer to the files in the order specified below for more details:

- [lab_instructions_part1.md](lab_instructions_part1.md)
- [lab_instructions_part2.md](lab_instructions_part2.md)Project lead: Member 4 — responsible for overall coordination.
